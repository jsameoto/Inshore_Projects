...
detail.dat <- read.csv(paste0(dir,"/Data/SFA29_MWSH_2000-2025_postAug15.csv")) ## we can use all data back to 98... update 
  
##Shapefiles
extent <- st_read(paste0(dir,"/GIS_data/SFA29_Extent.shp"))
...
ggplot(latt.1, aes(x = fit, y = res)) +
  ...
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/SFA29W/SFA29W.model.1.resids.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
ggplot(latt.1, aes(x = fit, y = WET_MEAT_WGT)) +
  ...
  labs(title = "Residuals vs Fitted", x = "Fitted Values", y = "Residuals")+
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/SFA29W/SFA29W.model.1.fit.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
Final.Data$Area <- "SFA29"

#--- Export final model and final model condition dataset ---- 

#output of final model 
#Save summary to txt file
sink(paste0(dir,"/ScxConditionModels/SFA29_ModelSummary.txt"))
print(summary(Final.Model))
sink()

#Condition dataset 
write.csv(Final.Data, paste0(dir, "/ScxConditionModels/SFA29_ConditionTS.csv"), row.names = FALSE)
