...
detail.dat <- read.csv(paste0(dir,"/Data/BRILUR_MWSH_1999-2025_Jun1-Aug15.csv")) 
  
##Shapefiles
extent <- st_read(paste0(dir,"/GIS_data/SPA3_BrierLurcher_Extent.shp"))
...
ggplot(latt.1, aes(x = fit, y = res)) +
  ...
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/SPA3_BL/SPA3_BL.model.1.resids.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
ggplot(latt.1, aes(x = fit, y = WET_MEAT_WGT)) +
  ...
  labs(title = "Residuals vs Fitted", x = "Fitted Values", y = "Residuals")+
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/SPA3_BL/SPA3_BL.model.1.fit.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
Final.Data$Area <- "SPA3_BrierLurcher"

#--- Export final model and final model condition dataset ---- 

#output of final model 
#Save summary to txt file
sink(paste0(dir,"/ScxConditionModels/SPA3_BrierLurcher_ModelSummary.txt"))
print(summary(Final.Model))
sink()

#Condition dataset 
write.csv(Final.Data, paste0(dir, "/ScxConditionModels/SPA3_BrierLurcher_ConditionTS.csv"), row.names = FALSE)
