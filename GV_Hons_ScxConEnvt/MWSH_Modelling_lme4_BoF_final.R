...
detail.dat <- read.csv(paste0(dir,"/Data/BF_MWSH_1998-2025_Jun1-Aug15.csv"))
  
##Shapefiles
extent <- st_read(paste0(dir,"/GIS_data/BoFextent.shp"))
...
ggplot(latt.1, aes(x = fit, y = res)) +
  ...
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/BoF/BoF.model.1.resids.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
ggplot(latt.1, aes(x = fit, y = WET_MEAT_WGT)) +
  ...
  labs(title = "Residuals vs Fitted", x = "Fitted Values", y = "Residuals")+
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/BoF/BoF.model.1.fit.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
sink(paste0(dir,"/ScxConditionModels/BF_ModelSummary.txt"))
print(summary(Final.Model))
sink()

#Condition dataset 
write.csv(Final.Data, paste0(dir, "/ScxConditionModels/BF_ConditionTS.csv"), row.names = FALSE)
