...
detail.dat <- read.csv(paste0(dir,"/Data/GM_MWSH_2006-2025_Jun1-Aug15.csv")) ## we can use all data back to 98... update 
  
##Shapefiles
#extent <- st_read("D:/GIthub/Inshore_Projects/GV_Hons_ScxConEnvt/BoFextent.shp")
extent <- st_read(paste0(dir,"/GIS_data/GMextent.shp"))
...
ggplot(latt.1, aes(x = fit, y = res)) +
  ...
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/GM/GM.model.1.resids.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
ggplot(latt.1, aes(x = fit, y = WET_MEAT_WGT)) +
  ...
  labs(title = "Residuals vs Fitted", x = "Fitted Values", y = "Residuals")+
  facet_wrap(~year)
ggsave(paste0(dir,"/ScxConditionModels/GM/GM.model.1.fit.tiff"), width = 15, height = 10, units = "cm", dpi = 300)
...
sink(paste0(dir,"/ScxConditionModels/GM_ModelSummary.txt"))
print(summary(Final.Model))
sink()

#Condition dataset 
write.csv(Final.Data, paste0(dir, "/ScxConditionModels/GM_ConditionTS.csv"), row.names = FALSE)
